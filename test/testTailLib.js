...
describe('tail', () => {
  ...
  it('Should give all lines starting from 2nd line', () => {
    const content = 'hello\nby\nhi\nbye';
    const expected = 'by\nhi\nbye';
    assert.strictEqual(tail(content, { option: '-n', count: '+2' }), expected);
  });

  it('Should give all lines starting from 5th line', () => {
    const content = 'a\nb\nc\nd\ne\nf\ng\nh\ni\nj\nk';
    const expected = 'e\nf\ng\nh\ni\nj\nk';
    assert.strictEqual(tail(content, { option: '-n', count: '+5' }), expected);
  });

  it('Should give all bytes starting from 2nd byte', () => {
    assert.strictEqual(tail('hello', { option: '-c', count: '+2' }), 'ello');
  });
});
